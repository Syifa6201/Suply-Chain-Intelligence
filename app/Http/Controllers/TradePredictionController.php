...
class TradePredictionController extends Controller
{
    ...
    )
    {
        ...
        foreach($countries as $country){
            ...
            $future = $analysis['score'] + rand(-5,10);



            if($future > $analysis['score']){

                $trend = "Increasing";

            }
            elseif($future < $analysis['score']){
        ...
        $totalCountries = count($predictions);


        $growing = collect($predictions)
            ->where('trend','Increasing')
            ->count();


        $stable = collect($predictions)
            ->where('trend','Stable')
            ->count();


        $declining = collect($predictions)
